...
The app.sh file is intended to be a file that organize the invocation of all of the files, so the user will only need to work with one file. This file will invoke OrganaziedGenerator.py then will activate the condo environment "bioactivity", then will invoke the app.py file with Streamlit package. 

NOTE: Every graph was excluded from this automation.

NOTE: The assumed structure is as follow: The three files and every file that was used in part 1 to part 6, that were essential to complete the tasks of these parts, are on one directory. And inside this directory, the bioactivity-prediction-app-main directory which includes everything in the original repository.